...
class KeychainService {
    ...
    ///     - group: Study Group
    func storeStudyToken(year: String?, major: String?, group: String?) {
        guard let year = year, let major = major, let group = group else {
        ...
        if let studyToken = "\(year):\(major):\(group)".data(using: .utf8) {
    ...
    /// - Returns:
    ///     - Tripple: Year, Major (Fach), Group
    func readStudyToken() -> (year: String?, major: String?, group: String?) {
        guard let studyToken = KeychainService.shared[.studyToken], let decodedStudyToken = Data(base64Encoded: studyToken)  else { return (nil, nil, nil) }
        guard let components = String(data: decodedStudyToken, encoding: .utf8)?.components(separatedBy: ":"), components.count == 3 else { return (nil, nil, nil) }
